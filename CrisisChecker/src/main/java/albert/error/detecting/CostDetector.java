...
public class CostDetector extends ErrorDetector
{
	...
	public CostDetector(Sheet sheet, Campaign campaign) {
		super(sheet, campaign);
		this.error = ErrorType.COST;
	...
	protected double source(Row row) {
		return error.format(sheet.cellValueDouble(row, "Budget"));
	}
}